...
const ProcessModal = ({ isOpen, onClose, currentStage, stages, title = "Process Running" }) => {
  if (!isOpen) return null;

  const getStageIcon = (stageIndex, currentStageIndex) => {
  ...
  const getStageStatus = (stageIndex, currentStageIndex) => {
    if (stageIndex < currentStageIndex) {
      return 'completed';
    } else if (stageIndex === currentStageIndex) {
      return 'running';
  ...
  const getConnectorStyles = (stageIndex, currentStageIndex) => {
    if (stageIndex < currentStageIndex) {
      return 'bg-green-400';
    } else {
      return 'bg-gray-300';
